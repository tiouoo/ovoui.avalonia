...
public class NavMenuItem : HeaderedItemsControl
{
    ...
    protected override void OnKeyDown(KeyEventArgs e)
    {
        ...
        if (!IsHorizontalCollapsed)
        {
            var handler = e.Key switch
            {
                ...
                Key.Enter => ApplyToItemOrRecursivelyIfCtrl(IsVerticalCollapsed ? ExpandItem : CollapseItem,
                    e.KeyModifiers),
            ...
            static Func<NavMenuItem, bool> ApplyToItemOrRecursivelyIfCtrl(Func<NavMenuItem, bool> f,
                KeyModifiers keyModifiers) => keyModifiers.HasFlag(KeyModifiers.Control)
                ? ApplyToSubtree(f)
                : f;
    ...
    internal void CloseAllOpenPopups()
    {
        ...
        foreach (var item in items)
            if (item._popup is { IsOpen: true } popup)
                popup.Close();
